...
in x, y, z co-ord
 From 000  there are 12 sites (pm,pm.0) (pm,0.pm) (0,pm,pm)
...
inline int tetra(int i, int mu)
{
  ...
  switch(mu)
    {
      //Tetrahedron in cube from i and refection
    case 0: imu = (i+1)%L   + ((i/L +1)%L)*L    + ((i/L/L)%L)*L*L;  break;         // + 110  1
    case 6: imu = (i-1+L)%L + ((i/L -1 +L)%L)*L + ((i/L/L)%L)*L*L;  break;     
      
    case 1: imu = i%L + ((i/L +1)%L)*L    + ((i/L/L+1)%L)*L*L;  break;            // + 011   2
    case 7: imu = i%L + ((i/L -1 +L)%L)*L + ((i/L/L -1 + L)%L)*L*L;  break;           
      
    case 2: imu = (i+1)%L   +  ((i/L)%L)*L + ((i/L/L +1)%L)*L*L;  break;           //   101  3
    case 8: imu = (i-1+L)%L +  ((i/L)%L)*L + ((i/L/L -1 +L)%L)*L*L;  break;     
      
    case 3: imu = (i-1+L)%L +  ((i/L)%L)*L + ((i/L/L +1)%L)*L*L;  break;           //   -1,0, 1 = 2-1 =  011 - 110  
    case 9: imu = (i+1)%L +    ((i/L)%L)*L + ((i/L/L -1 +L)%L)*L*L;  break;     
      
    case 4:  imu = (i+1)%L        + ((i/L-1 + L)%L)*L + ((i/L/L)%L)*L*L;  break;    // 1,-1, 0 =  3 -2 =   101 -  011 
    case 10: imu = (i -1 + L) %L +  ((i/L+1)%L)*L      + ((i/L/L)%L)*L*L;  break; 
      
    case 5:  imu = i%L +  ((i/L+1)%L)*L + ((i/L/L -1 + L)%L)*L*L;  break;           //  0,1,-1  = 1 - 3  110-  - 101 
    case 11: imu = i%L   +  ((i/L -1 +L)%L)*L + ((i/L/L +1)%L)*L*L;  break;  
...
int main()
{
  double truersq = 0.0;
  p.N = L*L*L;
  double *phi  = (double *) malloc(p.N* sizeof(double));
  double *phi0 = (double *) malloc(p.N*sizeof(double));
  ...
  for(int i = 0; i<p.N; i++)
    {
      cout << "\n i  = " << i << "    ";
       ...
       for(int mu = 0; mu < 6;mu++)
      	{
          cout <<  tetra(  tetra( i,  mu),  mu+6) - i  << "   "<<  tetra(  tetra( i,  mu+6),  mu) - i  << "   ";
  ...
  for(int i = 0; i<p.N; i++) {
   ...
   //  for(int i = 0; i<p.N; i++) cout << " i = " << i << "even = "<< even(i) << endl;
...
//  see http://en.wikipedia.org/wiki/Conjugate_gradient_method
  double  res[p.N], resNew[p.N],  pvec[p.N], Apvec[p.N];
  ...
  for(k = 0; k< maxIter; k++) {
    rsq = 0;
    for (i = 0; i < p.N; i++) rsq += res[i]*res[i];
     cout << endl << " rsq = " << rsq << endl;

    Aphi(Apvec, pvec);  
    denom = 0;
    for(i=0; i< p.N; i++) denom += pvec[i]*Apvec[i];
    alpha = rsq/denom;


    for(i=0; i < p.N; i++)  phi[i] +=  alpha * pvec[i];
    for(i=0; i < p.N; i++) resNew[i] = res[i]- alpha*Apvec[i];

    // Exit if new residual is small enough
    rsqNew = 0;
    for (i = 0; i < p.N; i++) rsqNew += resNew[i]*resNew[i];
  ...
  Aphi(Apvec,phi);
  for(i=0; i < p.N; i++) truersq += (Apvec[i] - b[i])*(Apvec[i] - b[i]);
